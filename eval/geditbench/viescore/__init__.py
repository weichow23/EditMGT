...
from eval.geditbench.viescore.utils import (
    mllm_output_to_dict
)
import math
import eval.geditbench.viescore.vie_prompts as vie_prompts
...
class VIEScore:
    def __init__(self, backbone="gpt4o", task="t2i", key_path=None) -> None:
        ...
        if self.backbone_name == "gpt4o":
            from eval.geditbench.viescore.mllm_tools.openai import GPT4o
            self.model = GPT4o(key_path, model_name="gpt-4.1")
        elif self.backbone_name == "gpt4v":
            from eval.geditbench.viescore.mllm_tools.openai import GPT4v
            self.model = GPT4v(key_path)
        elif self.backbone_name == 'gpt-4o':
            from eval.geditbench.viescore.mllm_tools.openai import GPT4v
            self.model = GPT4v(key_path, model_name='gpt-4o')
        elif self.backbone_name == 'gpt-4.1':
            from eval.geditbench.viescore.mllm_tools.openai import GPT4v
            self.model = GPT4v(key_path, model_name='gpt-4.1')
        elif self.backbone_name == "gemini":
            from eval.geditbench.viescore.mllm_tools.gemini import Gemini
            self.model = Gemini()
        elif self.backbone_name == "idefics2":
            from eval.geditbench.viescore.mllm_tools.idefics2_eval import Idefics2
            self.model = Idefics2()
        elif self.backbone_name == "mantis":
            from eval.geditbench.viescore.mllm_tools.mantis_idefics2_eval import Mantis
            self.model = Mantis()
        elif self.backbone_name == "minicpmv":
            from eval.geditbench.viescore.mllm_tools.minicpmv_eval import MiniCPMV
            self.model = MiniCPMV()
        elif self.backbone_name == "qwen25vl":
            from eval.geditbench.viescore.mllm_tools.qwen25vl_eval import Qwen25VL
            self.model = Qwen25VL()
        else:
    ...
    def evaluate(self, image_prompts, text_prompt, extract_overall_score_only=False, extract_all_score=True, echo_output=False):
        ...
        if self.backbone_name in ['gpt4o', 'gpt4v', 'gpt-4o', 'gpt-4.1']:
