...
'''
from eval.geditbench.viescore import VIEScore
import PIL
import os
import megfile
...
def process_single_item(item, vie_score, max_retries=10000):
    ...
    for retry in range(max_retries):
        try:
            ...
            except (IOError, OSError, Image.DecompressionBombError, Image.UnidentifiedImageError) as img_error:
                print(f"Error loading input image for key {key}: {img_error}")
                return None
            try:
                pil_image_edited = Image.open(megfile.smart_open(edit_image_path, 'rb')).convert("RGB")
            except (IOError, OSError, Image.DecompressionBombError, Image.UnidentifiedImageError) as img_error:
                print(f"Error loading edited image from {edit_image_path} for key {key}: {img_error}")
                return None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--backbone", type=str, default="gpt4o", choices=["gpt4.1","gpt4o", "qwen25vl"])
    ...
    if args.backbone == 'gpt4.1':
        vie_score = VIEScore(backbone='gpt-4.1', task="tie", key_path=<YOUR KEY>)
    elif args.backbone == 'gpt4o':
        vie_score = VIEScore(backbone='gpt-4o', task="tie", key_path=<YOUR KEY>)
